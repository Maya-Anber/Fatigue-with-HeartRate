...
import { EMGMonitor } from '../emg';
import { BarbellVelocityTracker } from '../barbell';
import { HeartRateMonitorServerProxy } from '../heart-rate';
import { FatigueEngine } from '../fatigue-engine';

// ─── Factory: build a wired-up system instance ────────────────────────────────

function buildSystem(hrMax: number) {
  ...
  const engine = new FatigueEngine(aggregator, { hrMax });
...
export function createAPIServer(port = 3001) {
  ...
  let session: ReturnType<typeof buildSystem> | null = null;
  ...
  app.post('/session/start', (req: Request, res: Response) => {
    const { hrMax = 185 } = req.body as { hrMax?: number };
  ...
  app.post('/session/set-done', (_req: Request, res: Response) => {
    if (!session) return res.status(400).json({ error: 'No active session' });
    session.engine.recordSetComplete();
    res.json({ ok: true });
  });

  app.post('/session/end', (_req: Request, res: Response) => {
    if (!session) return res.status(400).json({ error: 'No active session' });
  ...
  app.post('/ingest/hr', (req: Request, res: Response) => {
    if (!session) return res.status(400).json({ error: 'No active session' });
    const { bpm, confidence } = req.body as { bpm: number; confidence: number };
    session.hrProxy.ingest({ bpm, confidence, timestamp: Date.now() });
    res.json({ ok: true });
  });

  app.post('/ingest/emg', (req: Request, res: Response) => {
    if (!session) return res.status(400).json({ error: 'No active session' });
    const { samples } = req.body as { samples: number[] };
    session.emg.ingestRawSamples(samples);
    res.json({ ok: true });
  });

  app.post('/ingest/velocity', (req: Request, res: Response) => {
    if (!session) return res.status(400).json({ error: 'No active session' });
    const { velocityMps } = req.body as { velocityMps: number };
  ...
  wss.on('connection', (ws) => {
    ...
    ws.on('message', (raw) => {
      try {
        ...
        switch (msg.type) {
          case 'hr': {
            const hr = msg.data as { bpm: number; confidence: number };
            session.hrProxy.ingest({ ...hr, timestamp: Date.now() });
            break;
          }
          case 'emg': {
            const { samples } = msg.data as { samples: number[] };
            session.emg.ingestRawSamples(samples);
            break;
          }
          case 'velocity': {
            const { velocityMps } = msg.data as { velocityMps: number };
